...
2. Determine order of execution tasks in case when youre tasks with the same type and time of execution. I assume the better option is to execute in order of adding tasks - FIFO model.
3. Add execution policy - replace tasks if it exist or add it.
4. Add support of using of local time, not only UTC.
5. Separate the timer into a separate entity.
    1. Add possibility to tweak scheduler behaviour by using youre own timer.
    2. Add to CMake possibility to built without tests and SQLite support to have possibility to built library without any external dependency. Add example with this approach.
6. Provide multithreading use of Scheduler - make it possible to be used from different threads.
7. Provide multithread execution of the tasks.
